...
def train_subreward(dataloader, model, optimizer, criterion, scheduler, epoch, args):
    ...
    progress = ProgressMeter(
        ...
        logfile=os.path.join(args.save_folder, f'log_training_{args.model_name}.csv')
    )
    ...
    for idx, batch in enumerate(dataloader):
        ...
        inputs = {'input_ids': batch[0],
                  ...
                  'labels': batch[2]}  # Ajustado de batch[3] para batch[2], já que `token_type_ids` foi removido
...
def validate_subreward(dataloader, model, criterion, epoch, args):
    ...
    progress = ProgressMeter(
        ...
        prefix="Epoch: [{}]".format(epoch),
        logfile=os.path.join(args.save_folder, f'log_validation_{args.model_name}.csv')
    )
